<template>
  <div id="header" ref="header" v-loading="loading" element-loading-text="loading">
    ...
    <div class="header">
      ...
      <div class="nav">
        <div class="navLeft">
          <span>我的模板（共{{ SMSNum + SMSChangeNum }}条）</span>
      ...
      <div class="table">
        <div class="tableT">
          <div class="tableTLeft">
            <el-button size="mini" :class="{ active: isDisableTotal }" @click="handleClick()" round>全部 ({{ SMSNum + SMSChangeNum }})</el-button>
            <el-button size="mini" :class="{ active: isDisableText }" @click="handleClick(5)" round>普通模板 ({{ SMSNum }})</el-button>
            <el-button size="mini" :class="{ active: isDisableChange }" @click="handleClick(6)" round>变量模板 ({{ SMSChangeNum }})</el-button>
          </div>
          <div class="tableTRight">
            <el-button v-if="isListShow" type="text" @click.stop="addMessage" style="color: #fe9636" v-has="'smsMessageMouldAddOrUpdate'"><i class="el-icon-circle-plus-outline"></i>
              <p>新建模板</p>
            </el-button>
          ...
          <el-table ref="multipleTable" :data="tableData" :header-cell-style="{ background: '#F4F4F4', color: '#333333' }" border size="small" tooltip-effect="dark" style="width: 100%"
            ...
            <el-table-column prop="address" label="模版类型"  show-overflow-tooltip>
              <template slot-scope="scope">
                <span v-if="scope.row.messageType === 4">普通模板</span>
                <span v-if="scope.row.messageType === 5">变量模板</span>
              </template>
            </el-table-column>
            ...
            <el-table-column label="操作" width="130">
              <template slot-scope="scope">
                <!-- <el-tooltip class="item" effect="dark" content="编辑" placement="bottom">
                  <el-button @click.native.prevent="handleEdit(scope.$index, scope.row)" type="text" size="small" v-has="'smsMessageMouldAddOrUpdate'">
                    <img src="../../assets/svg/icon-24px-编辑@1x.svg" alt="" />
                  </el-button>
                </el-tooltip> -->
                <span class="operation" @click.prevent="handleEdit(scope.$index, scope.row)" v-has="'smsMessageMouldAddOrUpdate'">编辑</span>
                <!-- <el-tooltip class="item" effect="dark" content="删除" placement="bottom"> -->
                  <el-popover placement="bottom" width="200" :ref="`deleteVideo-${scope.$index}`" trigger="click">
                    ...
                    <!-- <el-button type="text" style="padding: 0" slot="reference" v-has="'smsMessageMouldDeleteById'">
                      <img src="../../assets/svg/icon-24px-删除@1x.svg" />
                    </el-button> -->
                    <span class="operation" slot="reference" v-has="'smsMessageMouldDeleteById'">删除</span>
                  </el-popover>
                <!-- </el-tooltip> -->
                <!-- <el-tooltip class="item" effect="dark" content="发送" placement="bottom">
                  <el-button @click.native.prevent="handleSend(scope.row)" type="text" size="small" v-has="'smsMessageMouldSendMouldMessage'" v-if="scope.row.checkStatus == 1">
                    <img src="../../assets/svg/icon-24px-发送@1x.svg" alt="" />
                  </el-button>
                  <el-button @click.native.prevent="handleSend(scope.row)" type="text" size="small" v-has="'smsMessageMouldSendMouldMessage'" v-else disabled>
                    <img src="../../assets/svg/icon-24px-发送@1x.svg" alt="" />
                  </el-button>
                </el-tooltip> -->
                <span class="operation" @click.prevent="handleSend(scope.row)" v-has="'smsMessageMouldSendMouldMessage'" v-if="scope.row.checkStatus == 1">发送</span>
                <!-- <el-button size="small" type="text" @click.native.prevent="handleSend(scope.row)" v-has="'smsMessageMouldSendMouldMessage'" v-else disabled>发送</el-button> -->
              </template>
            </el-table-column>
          </el-table>
        </div>
        <div class="tablist" :style="{minHeight:tabHeight}" v-if="!disableList">
          ...
          <template v-for="(item,index) in tableData">
            <!-- ----------------------------------------文本--------------------------------------------------- -->
            <div class="tablistBox" :key="item.id" v-if="item.messageType == 4">
              <div class="boxTitle">
                <span class="operation" @click.prevent="handleEdit(index, item)" v-has="'smsMessageMouldAddOrUpdate'">编辑</span>
                ...
                <span class="operation" @click.prevent="handleSend(item)" v-has="'smsMessageMouldSendMouldMessage'" v-if="item.checkStatus == 1">发送</span>
              </div>
            ...
            <!-- ----------------------------------------变量--------------------------------------------------- -->
            <div class="tablistBox" :key="item.id" v-if="item.messageType == 5">
              <div class="boxTitle">
                  <span class="operation" @click.prevent="handleEdit(index, item)" v-has="'smsMessageMouldAddOrUpdate'">编辑</span>
                <el-popover placement="bottom" :ref="`deleteVideo-${index}`" trigger="click">
                    <span style="padding: 30px">是否删除该消息模板？</span>
...
import {
  ...
  sentMessage,
  deleteMessageMould,
} from '../../api/sms/smsMessageTemplate'
      ...
      getMouldNum({})
        .then((res) => {
          if (res.data.status == 0) {
            ...
            this.SMSNum = res.data.data.SMS_NORMAL
            this.SMSChangeNum = res.data.data.SMS_VARIABLE
          } else {
            this.$message.error({
              message: res.data.message,
              center: true,
